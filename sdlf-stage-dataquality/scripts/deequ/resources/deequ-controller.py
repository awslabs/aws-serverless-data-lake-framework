...
def get_suggestions(table, key_value):
    response = table.query(IndexName="table-index", KeyConditionExpression=Key("table_hash_key").eq(key_value))
...
# Default Parameters
default_args = getResolvedOptions(sys.argv, ["env"])
env = default_args["env"]

# Required Parameters
args = getResolvedOptions(sys.argv, ["team", "dataset", "glueDatabase", "glueTables"])
...
suggestions_tables = []
verification_tables = []
suggestions_dynamo = dynamodb.Table("octagon-DataQualitySuggestions-{}".format(env))
for table in glue_tables:
    suggestions_item = get_suggestions(suggestions_dynamo, f"{team}-{dataset}-{table}")
    if suggestions_item:
