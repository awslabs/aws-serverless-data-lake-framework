...
This role can be used by CodeBuild projects hosted in a different AWS account.

Usage: ./deploy-generic-crossaccount.sh [-V | --version] [-h | --help] [-p | --profile <aws-profile>] [-c sdlf-construct1 | --construct sdlf-construct1 ] [-c sdlf-construct...] <name>
...
if [ -z ${1+x} ]; then die 'ERROR: "./deploy-generic-crossaccount.sh" requires a non-option argument.'; fi
...
STACK_NAME="sdlf-cicd-crossaccount-$1"
echo "CloudFormation stack name: $STACK_NAME"
aws cloudformation deploy \
    --stack-name "$STACK_NAME" \
    --template-file "$DIRNAME"/template-cicd-generic-crossaccount.yaml \
    --parameter-overrides \
        pCodeBuildSuffix="$1" \
    --tags Framework=sdlf \
