...
function setup_bbucket_workspacevars() {
  ...
  curl -X POST -H "Content-Type: application/json" -u ${BBUSER}:${APP_PASS} -d "{ \"type\": \"pipeline_variable\", \"key\": \"SDLF_AWS_REGION\", \"value\": \"${REGION}\", \"secured\": \"true\" }" https://api.bitbucket.org/2.0/workspaces/$WORKSPACE/pipelines-config/variables
