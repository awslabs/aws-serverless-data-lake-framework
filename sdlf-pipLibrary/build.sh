...
do
    ...
    if [ -f "./requirements.txt" ]; then
        ...
        set +e
        layer=$(aws lambda publish-layer-version --layer-name sdlf-$team_name-$dir_name --description "Contains the libraries specified in requirements.txt" --compatible-runtimes "python3.7" "python3.8" --zip-file fileb://./layer.zip)
        status=$?
        set -e
