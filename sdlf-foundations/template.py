...
class SdlfFoundations(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        if run_in_vpc:
            datalakeadmin_role_managed_policies.append(
                iam.ManagedPolicy.from_aws_managed_policy_name("service-role/AWSLambdaVPCAccessExecutionRole")
            )

        datalakeadmin_role_policy = iam.Policy(
            self,
            "sdlf-lakeformation-admin",
            statements=[
                iam.PolicyStatement(
                    actions=[
                        "logs:CreateLogGroup",
        ...
        ######## GLUE REPLICATION #########
        ######## LAMBDA #########
        replicate_function = _lambda.Function(
            self,
            "rLambdaReplicate",
            runtime=_lambda.Runtime.PYTHON_3_12,
        ...
        gluecatalog_events_rule = events.Rule(
            self,
            "rLambdaEventsRule",
            event_pattern=events.EventPattern(
                source=["aws.glue"],
