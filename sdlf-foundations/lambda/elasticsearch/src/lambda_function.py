...
def create(event, logs_client, logger):
    ...
    filter_name = ""
    try:
        filter_name = logs_client.describe_subscription_filters(
            logGroupName=log_group_name)['subscriptionFilters'][0]['filterName']
    except (KeyError, IndexError) as e:
        filter_name = "LambdaStream_" + destination_lambda_name
        logger.info(
            "Cannot find existing subscription filter: %s. Created filter name: %s", e, filter_name)
