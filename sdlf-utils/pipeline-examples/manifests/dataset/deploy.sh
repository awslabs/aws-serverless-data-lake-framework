...
MANIFEST_ENABLED=$(sed -e 's/^"//' -e 's/"$//' <<<"$(jq '.[] | select(.ParameterKey=="pManifestFlag") | .ParameterValue' $DIRNAME/parameters-$ENV.json)")
MANIFEST_FILENAME=$(sed -e 's/^"//' -e 's/"$//' <<<"$(jq '.[] | select(.ParameterKey=="pManifestFileName") | .ParameterValue' $DIRNAME/parameters-$ENV.json)")
MANIFEST_FILEEXT=$(sed -e 's/^"//' -e 's/"$//' <<<"$(jq '.[] | select(.ParameterKey=="pManifestFileExtension") | .ParameterValue' $DIRNAME/parameters-$ENV.json)")
MANIFEST_FILEREGEX=$(sed -e 's/^"//' -e 's/"$//' <<<"$(jq '.[] | select(.ParameterKey=="pRegexPattern") | .ParameterValue' $DIRNAME/parameters-$ENV.json)")
MANIFEST_TIMEOUT=$(sed -e 's/^"//' -e 's/"$//' <<<"$(jq '.[] | select(.ParameterKey=="pManifestFileTimeout") | .ParameterValue' $DIRNAME/parameters-$ENV.json)")
MANIFEST_DATATIMEOUT=$(sed -e 's/^"//' -e 's/"$//' <<<"$(jq '.[] | select(.ParameterKey=="pDataFileTimeout") | .ParameterValue' $DIRNAME/parameters-$ENV.json)")
if ! $sflag
then
...
if ! aws cloudformation describe-stacks --profile $PROFILE --stack-name $STACK_NAME; then
  ...
  ### Changes for Manifest File Processing ###

  if [[ "${MANIFEST_ENABLED}" != "" && "${MANIFEST_ENABLED}" == "True" ]]
then
  DATASET_JSON=$( jq -n \
                --arg dn "$TEAM_NAME-$DATASET_NAME" \
                --arg pn "$PIPELINE_NAME" \
                --arg mf "$MANIFEST_ENABLED" \
                --arg mfn "$MANIFEST_FILENAME" \
                --arg mfe "$MANIFEST_FILEEXT" \
