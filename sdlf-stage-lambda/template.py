...
class SdlfStageLambda(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        p_pipeline = CfnParameter(self, "pPipeline",
        ...
        p_enabletracing = CfnParameter(self, "pEnableTracing",
            description="Flag for whether XRay tracing is enabled",
            type="String",
        )

        ######## IAM #########
        common_policy = iam.ManagedPolicy(self, "rLambdaCommonPolicy",
        ...
        ######## LAMBDA FUNCTIONS #########
        transformstep_role_policy = iam.Policy(self, "sdlf-transform-lambda", # f"sdlf-{p_teamname.value_as_string}-{p_pipeline.value_as_string}-process-{p_stagename.value_as_string}",
            statements=[
                iam.PolicyStatement(
                ...
                iam.PolicyStatement(
                    ...
                    resources=[
                        f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/DataKeyId}}}}"
                    ]
                ),
            ]
        )

        transformstep_role = iam.Role(self, "rRoleLambdaExecutionProcessingStep",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com"),
            path=f"/sdlf-{p_teamname.value_as_string}/",
            permissions_boundary=iam.ManagedPolicy.from_managed_policy_arn(self, "chaipa4", managed_policy_arn=f"{{{{resolve:ssm:/SDLF/IAM/{p_teamname.value_as_string}/TeamPermissionsBoundary}}}}")
        )
        transformstep_role.attach_inline_policy(transformstep_role_policy)
        transformstep_role.add_managed_policy(common_policy)

        transformstep_function = _lambda.Function(self, "rLambdaTransformStep",
            ...
            environment_encryption=kms.Key.from_key_arn(self, "chaipakms", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}"),
            # vpcconfig TODO
        )

        logs.LogGroup(self, "rLambdaTransformStepLogGroup",
...
#            retention=Duration.days(p_cloudwatchlogsretentionindays.value_as_number),
            encryption_key=kms.Key.from_key_arn(self, "chaipakms2", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}"),
        )



        # Metadata Step Role (fetch metadata, update pipeline execution history...)
        postmetadatastep_role_policy = iam.Policy(self, "sdlf-metadata-lambda", # f"sdlf-{p_teamname.value_as_string}-{p_pipeline.value_as_string}-metadata-{p_stagename.value_as_string}",
            statements=[
                iam.PolicyStatement(
        ...
        postmetadatastep_role = iam.Role(self, "rRoleLambdaExecutionMetadataStep",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com"),
            path=f"/sdlf-{p_teamname.value_as_string}/",
            permissions_boundary=iam.ManagedPolicy.from_managed_policy_arn(self, "chaipa1", managed_policy_arn=f"{{{{resolve:ssm:/SDLF/IAM/{p_teamname.value_as_string}/TeamPermissionsBoundary}}}}")
        )
        ...
        postmetadatastep_function = _lambda.Function(self, "rLambdaPostMetadataStep",
            runtime = _lambda.Runtime.PYTHON_3_12,
            ...
            environment_encryption=kms.Key.from_key_arn(self, "chaipakms3", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}")
            # vpcconfig TODO
        )

        logs.LogGroup(self, "rLambdaPostMetadataStepLogGroup",
            log_group_name=f"/aws/lambda/{postmetadatastep_function.function_name}",
            retention=logs.RetentionDays.ONE_MONTH,
#            retention=Duration.days(p_cloudwatchlogsretentionindays.value_as_number),
            encryption_key=kms.Key.from_key_arn(self, "chaipakms4", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}")
        )


        errorstep_role_policy = iam.Policy(self, "sdlf-error-lambda", # f"sdlf-{p_teamname.value_as_string}-{p_pipeline.value_as_string}-error-{p_stagename.value_as_string}",
            statements=[
                iam.PolicyStatement(
                    actions=[
                        "sqs:DeleteMessage",
        ...
        errorstep_role = iam.Role(self, "rRoleLambdaExecutionErrorStep",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com"),
            path=f"/sdlf-{p_teamname.value_as_string}/",
            permissions_boundary=iam.ManagedPolicy.from_managed_policy_arn(self, "chaipa2", managed_policy_arn=f"{{{{resolve:ssm:/SDLF/IAM/{p_teamname.value_as_string}/TeamPermissionsBoundary}}}}")
        )
        errorstep_role.attach_inline_policy(errorstep_role_policy)
        errorstep_role.add_managed_policy(common_policy)

        errorstep_function = _lambda.Function(self, "rLambdaErrorStep",
            runtime = _lambda.Runtime.PYTHON_3_12,
            ...
            timeout=Duration.seconds(300),
            role=errorstep_role,
            environment_encryption=kms.Key.from_key_arn(self, "chaipakms5", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}")
            # vpcconfig TODO
        )

        logs.LogGroup(self, "rLambdaErrorStepLogGroup",
            log_group_name=f"/aws/lambda/{errorstep_function.function_name}",
            retention=logs.RetentionDays.ONE_MONTH,
#            retention=Duration.days(p_cloudwatchlogsretentionindays.value_as_number),
            encryption_key=kms.Key.from_key_arn(self, "chaipakms6", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}")
        )
...
#           Value: !Ref rLambdaErrorStep
        ...
        ######## STATE MACHINE #########
        statemachine_role_policy = iam.Policy(self, "sdlf-statemachine-lambda", # f"sdlf-{p_teamname.value_as_string}-states-execution",
            statements=[
                iam.PolicyStatement(
        ...
        statemachine_role = iam.Role(self, "rStatesExecutionRole",
            assumed_by=iam.PrincipalWithConditions(
                iam.ServicePrincipal("states.amazonaws.com", region=self.region),
                conditions={"StringEquals": {"aws:SourceAccount": self.account}}
            ),
            path=f"/sdlf-{p_teamname.value_as_string}/",
            permissions_boundary=iam.ManagedPolicy.from_managed_policy_arn(self, "chaipa3", managed_policy_arn=f"{{{{resolve:ssm:/SDLF/IAM/{p_teamname.value_as_string}/TeamPermissionsBoundary}}}}")
        )
        statemachine_role.attach_inline_policy(statemachine_role_policy)

        statemachine = sfn.StateMachine(self, "rStateMachine",
        ...
        ssm.StringParameter(self, "rStateMachineSsm",
        ...
        statemachine_map_role_policy = iam.Policy(self, "sfn-map",
            statements=[
                iam.PolicyStatement(
                    actions=[
        ...
        ### Routing/Redrive
        routingstep_role_policy = iam.Policy(self, "sdlf-routing-lambda", # f"sdlf-{p_teamname.value_as_string}-{p_pipeline.value_as_string}-routing-{p_stagename.value_as_string}",
            statements=[
                iam.PolicyStatement(
                    actions=[
                        "sqs:DeleteMessage",
        ...
        routingstep_role = iam.Role(self, "rRoleLambdaExecutionRoutingStep",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com"),
            path=f"/sdlf-{p_teamname.value_as_string}/",
            permissions_boundary=iam.ManagedPolicy.from_managed_policy_arn(self, "chaipa5", managed_policy_arn=f"{{{{resolve:ssm:/SDLF/IAM/{p_teamname.value_as_string}/TeamPermissionsBoundary}}}}")
        )
        routingstep_role.attach_inline_policy(routingstep_role_policy)
        routingstep_role.add_managed_policy(common_policy)

        routingstep_function = _lambda.Function(self, "rLambdaRoutingStep",
            runtime = _lambda.Runtime.PYTHON_3_12,
            ...
            environment_encryption=kms.Key.from_key_arn(self, "chaipakms7", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}")
            # vpcconfig TODO
        )
        ssm.StringParameter(self, "rRoutingLambdaSsm",
        ...
        logs.LogGroup(self, "rLambdaRoutingStepLogGroup",
            log_group_name=f"/aws/lambda/{routingstep_function.function_name}",
            retention=logs.RetentionDays.ONE_MONTH,
#            retention=Duration.days(p_cloudwatchlogsretentionindays.value_as_number),
            encryption_key=kms.Key.from_key_arn(self, "chaipakms8", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}")
        )

        redrivestep_function = _lambda.Function(self, "rLambdaRedriveStep",
            runtime = _lambda.Runtime.PYTHON_3_12,
            ...
            timeout=Duration.seconds(300),
            role=routingstep_role,
            environment_encryption=kms.Key.from_key_arn(self, "chaipakms9", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}")
            # vpcconfig TODO
        )

        logs.LogGroup(self, "rLambdaRedriveStepLogGroup",
            log_group_name=f"/aws/lambda/{redrivestep_function.function_name}",
            retention=logs.RetentionDays.ONE_MONTH,
#            retention=Duration.days(p_cloudwatchlogsretentionindays.value_as_number),
            encryption_key=kms.Key.from_key_arn(self, "chaipakms10", key_arn=f"{{{{resolve:ssm:/SDLF/KMS/{p_teamname.value_as_string}/InfraKeyId}}}}")
        )
