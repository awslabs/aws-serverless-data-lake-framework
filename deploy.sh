...
devops_account () { 
    ...
    [ "$(aws --region "$REGION" --profile "$DEVOPS_AWS_PROFILE" ssm get-parameter --name /SDLF/Monitoring/Enabled --query "Parameter.Value" --output text)" == "true" ] && REPOSITORIES+=("sdlf-monitoring")
